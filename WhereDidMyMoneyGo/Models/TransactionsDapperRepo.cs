...
namespace WhereDidMyMoneyGo.Models
{
    public class TransactionsDapperRepo
    {
        ...
        {
            return _connection.Query<TransactionsTable>("Select t.transactiondate as TransactionDate, v.vendorname as VendorName, c.categoryname as CategoryName, t.transactiontype as TransactionType, t.transactionamount as TransactionAmount From transactions as t Inner Join vendors as v on t.vendorid = v.vendorid Inner Join categories as c on t.categoryid = c.categoryid Where t.userid = @userId Order By TransactionDate desc, VendorName, CategoryName, TransactionType, TransactionAmount;",
                   new { userId = userId});
        }
